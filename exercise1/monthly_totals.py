...
def date_to_month(d):
    ...
    #return '%04i-%02i' % (d.year, d.month) # uncomment to use with loops
    
    # uncomment to use with pandas pandas
    def format_month(x):
        return '%02i' % x
    def format_year(x):
        return '%04i' % x
    month =d.dt.month.apply(format_month)
    year =d.dt.year.apply(format_year)

    return year.str.cat(month,sep='-')
...
def pivot_months_pandas(data):
    ...
    function to the existing 'date' column. [You may have to modify date_to_month 
    slightly, depending how your data types work out. ]
    '''
    ...
    aggregate_sum = grouped_data.aggregate('sum').reset_index()
    aggregate_count = grouped_data.aggregate('count').reset_index()

    '''Use the Pandas pivot method to create a row for each station (name) and column
     for each month.
    '''
    monthly = aggregate_sum.pivot(index='name', columns='month')['precipitation']
    counts = aggregate_count.pivot(index='name', columns='month')['precipitation']
